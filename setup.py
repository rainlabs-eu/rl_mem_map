...
DIRECTORIES_TO_REMOVAL = ["bloat-master","webtreemap-gh-pages"]
...
def delete_temporary_files(path_to_script_directory = None):
    ...
    files_to_remove = DIRECTORIES_TO_REMOVAL
...
def make_zip_file(path_to_script_directory):
    ...
    '''
    if not os.path.exists(bloat_folder_path):
        os.makedirs(bloat_folder_path)
    webtreemap_subfolder_path = os.path.join(bloat_folder_path,"webtreemap")
    if not os.path.exists(webtreemap_subfolder_path):
        os.makedirs(webtreemap_subfolder_path)
    shutil.copy(os.path.join(path_to_script_directory,FILES_TO_BE_ZIPPED[0]), path_to_script_directory)
    for i in [1,2]:
        shutil.copy(os.path.join(path_to_script_directory,FILES_TO_BE_ZIPPED[i]), webtreemap_subfolder_path)
    '''
